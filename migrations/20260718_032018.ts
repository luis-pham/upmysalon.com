...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_legal_pages_slug" AS ENUM('privacy', 'terms', 'sms-consent', 'refund');
  ...
  CREATE TABLE "legal_pages" (
  	...
  	"body_vi" jsonb NOT NULL,
  	"body_en" jsonb NOT NULL,
  	"seo_vi_title" varchar NOT NULL,
  	"seo_vi_description" varchar NOT NULL,
  	"seo_en_title" varchar NOT NULL,
  	"seo_en_description" varchar NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
